// MIT, 2015-2019, WinterDev, EngineKit, brezza92
#include <string>
...
void ResultSetManagedObjectIndex(MetCallingArgs* callingArgs,
                                 int32_t managedObjectIndex) {
  // TODO: review here
...
ManagedRef* JsContext::CreateWrapperForManagedObject(
    int mIndex, ExternalTypeDefinition* externalTypeDef) {
  ...
  if (externalTypeDef) {
    Local<ObjectTemplate> objTemplate = Local<ObjectTemplate>::New(
        isolate_, externalTypeDef->handlerToJsObjectTemplate);
    Local<Object> instance = objTemplate->NewInstance(ctx).ToLocalChecked();
    handler->v8InstanceHandler.Reset(isolate_, instance);
    Local<Object> hd =
        Local<v8::Object>::New(isolate_, handler->v8InstanceHandler);
...
ManagedRef* CreateWrapperForManagedObject(
    JsContext* engineContext,
    int mIndex,
    ExternalTypeDefinition* externalTypeDef) {
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void DoGetterProperty(Local<String> propertyName,
...
void DoSetterProperty(Local<String> propertyName,
  ...
  v8::Local<Object> obj =
      v8::Local<Object>::Cast<Value>(info.Holder()->GetInternalField(0));
  MetCallingArgs callingArgs;
  memset(&callingArgs, 0, sizeof(MetCallingArgs));
  callingArgs.accessorInfo = &info;
...
ExternalTypeDefinition* JsContext::RegisterTypeDefinition(int mIndex,
                                                          const char* stream,
                                                          int streamLength) {
  Locker locker(isolate_);
  Isolate::Scope isolate_scope(isolate_);
  ...
  Local<ObjectTemplate> objTemplate = ObjectTemplate::New(isolate_);
...
void ContextRegisterManagedCallback(JsContext* jsContext,
                                    void* callback,
                                    int callBackKind) {
  ...
  switch (args->methodCallKind) {
    case MET_SETTER: {
      ...
      Local<v8::External> ext =
          Local<v8::External>::Cast(args->accessorInfo->Data());
      v8::Local<Object> obj =
          v8::Local<Object>::Cast(args->accessorInfo->This());
      CallingContext* cctx = (CallingContext*)ext->Value();
    ...
    case MET_: {
      ...
      v8::Local<Object> obj = v8::Local<Object>::Cast(args->args->This());
...
void ArgGetThis(MetCallingArgs* args, jsvalue* output) {
  if (args->accessorInfo == NULL) {
    ...
    v8::Local<Object> obj = v8::Local<Object>::Cast(args->args->This());
    cctx->ctx->ConvAnyFromV8(obj, v8::Local<Object>(), output);
    return;
  } else {
    // use accessor
    // for getter/setter
    ...
    v8::Local<Object> obj = v8::Local<Object>::Cast(args->accessorInfo->This());
...
void ExternalTypeDefinition::ReadTypeDefinitionFromStream(
