...
static void managed_prop_get(Local<Name> name,
	...
	info.GetReturnValue().Set(ref->GetPropertyValue(name->ToString(isolate)));
}
...
static void managed_prop_set(Local<Name> name,
	...
	info.GetReturnValue().Set(
		Local<Value>::New(isolate, ref->SetPropertyValue(name->ToString(isolate), value)));
}
...
static void managed_prop_delete(Local<Name> name,
	...
	ManagedRef* ref = (ManagedRef*)wrap->Value();
	info.GetReturnValue().Set(
		Local<Boolean>::New(isolate, ref->DeleteProperty(name->ToString(isolate))));
}
...
void JsEngine::AnyFromV8(Handle<Value> value,
	...
	if (value->IsNull() || value->IsUndefined()) {
		...
		output->i32 = value->BooleanValue(isolate) ? 1 : 0;
	} else if (value->IsInt32()) {
		output->type = JSVALUE_TYPE_INTEGER;
		output->i32 = value->Int32Value(context).ToChecked();
	} else if (value->IsUint32()) {
		output->type = JSVALUE_TYPE_INDEX;
		output->i64 = value->Uint32Value(context).ToChecked();
	} else if (value->IsNumber()) {
		output->type = JSVALUE_TYPE_NUMBER;
		output->num = value->NumberValue(context).ToChecked();
	} else if (value->IsString()) {
		StringFromV8(value, output);
	} else if (value->IsDate()) {
		output->type = JSVALUE_TYPE_DATE;
		// v.value.num = value->NumberValue();
		output->i64 = value->IntegerValue(context).ToChecked();
	} else if (value->IsArray()) {
		Handle<Array> object = Handle<Array>::Cast(value->ToObject(isolate));
...
Handle<Value> JsEngine::AnyToV8(jsvalue* v, int32_t contextId) {
	...
	case JSVALUE_TYPE_MANAGED_ERROR: {
		...
		Local<Object> object = ((FunctionTemplate*)managed_template_)
			->InstanceTemplate()
			->NewInstance(isolate_->GetCurrentContext()).ToLocalChecked();
		if (object.IsEmpty()) {
			return Null(isolate_);
		}
